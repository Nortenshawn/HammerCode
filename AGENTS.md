...
- API key 的持久化读取和模型使用只能发生在 main process。用户在设置页输入自定义 API key 时，允许它短暂存在于密码输入框，并通过单向、带类型的 IPC 请求传入 main process；preload 和 main process 不得把密钥值回传 renderer，提交后 renderer 必须清空输入状态。
...
- 设置页保存的自定义 API key 必须使用 Electron `safeStorage` 加密后写入应用用户数据目录；系统安全存储不可用时只能执行当前连接测试，不得降级为明文持久化。公开连接信息仅包含脱敏 URL、模型列表、检测状态和时间。
