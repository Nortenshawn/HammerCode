...
const configSchema = z.object({
  ...
  maxConcurrentMainAgents: z.number().int().min(1).max(3),
...
export function loadRuntimeConfig(): RuntimeConfig {
  ...
  const parsed = configSchema.safeParse({
    ...
    maxConcurrentMainAgents: readInteger(process.env.HAMMERCODE_MAX_CONCURRENT_MAIN_AGENTS, 3),
...
export function toPublicConfig(
  ...
  return {
    ...
    maxConcurrentMainAgents: config.maxConcurrentMainAgents,
